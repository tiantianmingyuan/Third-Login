...
public class PropertiesLoader {
    ...
    public PropertiesLoader(String... resourcesPaths) {
        properties = loadPropertiesByActiveProfile(resourcesPaths);
    }
    ...
    private Properties loadPropertiesByActiveProfile(String... resourcesPaths) {
        String env = SpringContextHolder.getCurrentEnviroment();

        String[] actualResourcesPaths = new String[resourcesPaths.length];
        for (int i = 0; i < resourcesPaths.length; i++) {
            String location = resourcesPaths[i];
            if (StringUtils.isNotEmpty(env)) {
                location = FilenameUtils.getFullPath(location) + FilenameUtils.getBaseName(location) + "." + env + "."
                        + FilenameUtils.getExtension(location);
            }
            actualResourcesPaths[i] = location;
        }
        return loadProperties(actualResourcesPaths);
    }
